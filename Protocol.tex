...
Here is our new protocol.\mattia{I still need to finish this.% I think we can send the product of $\hat{K}_i^\text{se}$ and $\hat{K}_i^\text{ee}$ instead of sending them separately, so we can compute that term as $\left(\dfrac{E_{i+1}}{E_{i-1}} \right)^{\esk_i + \ssk_i}$ 
}
